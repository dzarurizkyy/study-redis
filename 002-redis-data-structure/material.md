...
```
**Note:** Adds a new entry with an auto-generated ID ("*" uses current timestamp).
...
```
**Note:** Reads all entries starting from the beginning (ID 0).
...
```
**Note:** Reads up to 'value' number of entries from the stream, starting from ID 0. Useful when you only want to fetch a limited number of messages at a time.
...
```
**Note:** Blocks the connection and waits indefinitely (0 = no timeout) for new entries. Useful for implementing real-time consumers or message listeners. When new data arrives, Redis immediately returns it to the client.
...
```
**Note:** Registers a new consumer (member) inside the given consumer group. Useful when adding multiple consumers that will share the stream workload.
